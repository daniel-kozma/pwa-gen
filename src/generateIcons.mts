...
export default async function generateIcons(publicDir: string, favicon: string, iconsDir = "icons", iconsFile = "icons.json", splashscreensFile = "splashscreens.html"): Promise<{
  icons: {
    src: string
    sizes: string
    type: string
  ...
  const stdout = await cmd(
    ...
    `npx pwa-asset-generator ${favicon} ${iconsDir} -m ${iconsFile} -i ${splashscreensFile} -s false`,
  ).catch(e => console.error(`[ERROR] Error while generating icons:`, e, "\n Continuing anyways"))
