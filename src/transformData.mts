...
import { CompiledData, InputData } from "./types.mjs";

const transformAsyncData = async (inp: InputData, publicDir: string): Promise<Partial<CompiledData>> => {
...
const transformSyncData = (inp: InputData): Partial<CompiledData> => ({
  lang: inp.lang || "en",
...
const transformData = async (inp: InputData, publicDir: string): Promise<CompiledData> =>
  ({
