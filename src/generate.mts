...
async function generate(publicDir: string, compileDataJsonFile: string, templateFile: string | null = null){
  ...
  const inputData: InputData = JSON.parse(
    await readFile(inputJsonFilePath, "utf-8")
  )
  ...
  const newHTML = await generateHTML(
    compiledData,
    templateFile === null ? defaultTemplate : await readFile(templateFile, "utf-8")
  )
